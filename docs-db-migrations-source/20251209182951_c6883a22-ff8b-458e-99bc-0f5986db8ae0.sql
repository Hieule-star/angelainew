...
-- Knowledge topics RLS policies (public read)
...
-- Function to update light points
CREATE OR REPLACE FUNCTION public.increment_light_points(_user_id UUID, _amount INTEGER DEFAULT 1)
RETURNS INTEGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  SET light_points = light_points + _amount,
      updated_at = NOW()
  WHERE id = _user_id
  RETURNING light_points INTO new_points;
